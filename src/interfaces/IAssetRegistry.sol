...
interface IAssetRegistry {
    ...
    /// @notice Emitted when trustless DvP fills (psmFillOrder) are paused or resumed for a wrapper.
    /// @param ticker  Asset ticker.
    /// @param wrapper Wrapper token address.
    /// @param paused  New pause flag.
    event PsmFillPausedUpdated(bytes32 indexed ticker, address indexed wrapper, bool paused);
    ...
    /// @notice Pause or resume trustless DvP fills (psmFillOrder) for one wrapper — the fill
    ///         channel only; psmMint/psmRedeem and the RFQ paths are unaffected. Default false =
    ///         live. Operator-only (instant response).
    /// @param ticker  Asset ticker.
    /// @param wrapper Wrapper token address.
    /// @param paused  New pause flag.
    function setPsmFillPaused(bytes32 ticker, address wrapper, bool paused) external;
