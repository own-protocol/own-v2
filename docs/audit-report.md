...
**Branch:** `lending` · **Last updated:** 2026-06-12 · **Test suite:** 636 passing
...
| Critical | 1 | 1 | 0 | — |
| High | 5 | 5 | 0 | — |
| Medium | 10 | 1 (M-05) | 8 | 1 (M-04) |
| Low | 13 | 0 | 13 | — |

**All Critical/High findings are fixed and regression-tested. Remaining work: 8 Mediums (M-01–M-03, M-06–M-10), 13 Lows, and the unconfirmed leads in §5.**
...
| M-03 | Medium | `setRateParams` unvalidated → accrual DoS bricks repay/liquidate | **Open** |
...
| M-06 | Medium | `borrow` checks the debt cap before `_accrue()` → cap bypass | **Open** |
| M-07 | Medium | `borrow` ignores the bound vault's Paused/Halted status | **Open** |
...
| M-10 | Medium | `WstETHRouter` wraps requested (not received) stETH → deposit path DoS | **Open** |
...
re-resolved the *current* active token. After a split migration, the market held the old token while
...
- **M-01 — Pyth confidence interval ignored.** `PythOracleVerifier._normalizePythPrice` uses only
  ...
  *Fix:* reject if `conf · BPS / price > maxConfBps`.
- **M-02 — Unset oracle config accepts unbounded prices.** `OracleVerifier` gates staleness and
  ...
  single push. *Fix:* require non-zero `maxStaleness`/`maxDeviation` before accepting prices.
- **M-03 — `setRateParams` unvalidated → accrual DoS.** `InterestRateModel.premium` reverts when
  `optimalUtilBps == 0 || >= BPS`; bad params (setter or constructor) make `_accrue` revert, which
  bricks repay and liquidate too. *Fix:* validate params in the setter and constructor.
- **M-06 — Debt cap checked pre-accrual.** `borrow` runs the hard cap against `totalDebtUSD()`
  *before* `_accrue()`, so accrued interest and the M-05 floor are excluded — a borrow that should
  breach the cap can pass. *Fix:* move `_accrue()` above the cap check.
- **M-07 — `borrow` ignores vault status.** `_validateEligibility` checks the asset only, never
  `vaultStatus()` — a paused/halted vault keeps being borrowed against via its Aave credit
  delegation (sibling of H-01). *Fix:* revert unless `vaultStatus() == Active`.
- **M-08 — `_flooredIndex` dust inflation / multi-manager corruption.** The M-05 floor divides
  ...
  debt. *Fix:* floor against this manager's own originated debt, or enforce a one-manager invariant
...
- **M-09 — Sub-unit amounts record zero scaled debt.** `actualToScaled` floors to 0 for small
  ...
  *Fix:* revert when `scaledRepay == 0` / `scaledDebt == 0` (and `seize == 0`).
- **M-10 — `WstETHRouter` wraps requested, not received, stETH.** stETH share-rounding delivers
  1–2 wei less than `transferFrom` requested, so `wstETH.wrap(stETHAmount)` reverts — breaking the
  stETH deposit path. *Fix:* wrap `stETH.balanceOf(this) − balBefore` (the balance-diff pattern
  `AaveRouter.deposit` already uses).

### Recommended order

1. M-10 (user-facing DoS, trivial fix) and M-03 / M-06 / M-07 (small guards in BorrowManager).
2. M-01 / M-02 (oracle hardening), M-09 (dust guards).
3. M-08 (needs a design decision on the floor's debt base).
...
- **C-01\* — Force-execute asset proof is window-scoped, not fresh (team-confirmed 2026-06-11).**
  ...
  `claimThreshold`, the user proves the asset reached their `limitPrice` *at some point in the
  order's `[createdAt, now]` window* and is filled at that pre-committed limit — claimable only by
...
- **L-13** A USDC/USDT blocklist freeze of a recipient permanently bricks `cancelOrder`/`expireOrder` for that order, and a frozen `haltRedeemAddress` bricks all halt redemptions for the asset. *Fix:* pull-payment fallback; monitored, non-freezable halt address.
...
*Original findings cite code at review time (2026-06-09 review at commit `fa9cf33`; re-audits on `lending` through 2026-06-11). Every Critical/High was verified directly against source, and every fix has a regression test that fails without it. Suite: 636 passing.*
